...
public class BuyCommand extends EconomyCommand {
	...
	public void onCommand(Player player, String[] args) {
		if (args.length != 3) {
			player.sendMessage(ChatColor.RED + "Incorrect syntax. Use /buy (amount) (item) (price)");
			return;
		}
		int amount;
		...
		RaidPlugin.getInstance().getEconomy().getMarket().buy(player, material, amount, RaidPlugin.getInstance().getEconomy().toValue(price));
	}
}
